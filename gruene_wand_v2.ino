...
const unsigned long messungsInterval_UL = 1u*1u*1000;//10*1000; // 24*60*60*1000;  // 1x in der Stunde messen und rausschreiben

const unsigned int pumpdauer = 1; // in Sekunden
const unsigned int pause = 2; // in Sekunden
const unsigned int schwellenwert = 500;

const unsigned int lichtAn = 9; // 9 uhr geht licht an
const unsigned int lichtAus = 17; // 17 uhr geht licht an
...
void loop()
{
  ...
  // Messen
  int sensorWert = analogRead(SENSORPIN); // und lies den aktuellen Sensorwert vor dem Wässern aus
  ...
  while (sensorWert>schwellenwert) { // wenn der Sensor über dem Schwellenwert liegt so lange gießen und warten bis er wieder drüber ist
    ...
    delay(pause*1000);
    sensorWert = analogRead(SENSORPIN);  // lies den aktuellen Sensorwert nach dem Wässern aus
